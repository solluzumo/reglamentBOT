...
async def process_title(message: types.Message):
    data = open(
        "static/text/reglaments/ИНСТРУКЦИЯ ООО «РН-ЮГАНСКНЕФТЕГАЗ» ПО ПРЕДУПРЕЖДЕНИЮ ВОЗНИКНОВЕНИЯ ГАЗОНЕФТЕВОДОПРОЯВЛЕНИЙ И ОТКРЫТЫХ ФОНТАНОВ ПРИ БУРЕНИИ НЕФТЯНЫХ И ГАЗОВЫХ СКВАЖИН", encoding="utf-8").read()
