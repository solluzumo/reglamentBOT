...
async def main(message: types.Message):
    ...
    for best_match in best_matches:
        ...
        parsed_title_file = json.loads(open(f'static/text/reglaments/{parsed_title_info[2]}').read())
    ...
    await title_handler.send_reply(message,parsed_title,cluster,is_cluster)
...
async def send_table(callback_query: types.CallbackQuery):
    ...
    table = open(f'static/tables/{parsed_title.id}/{parsed_title.table[int(index)]}', 'r').read()
